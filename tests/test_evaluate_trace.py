...
def test_run_eval_records_generation_failure_and_continues(tmp_path, monkeypatch) -> None:
    question = EvalQuestion(
        id="q1",
        question="Quel est le préavis ?",
        reference_answer="Le préavis est de deux mois.",
    ...
    def fail_answer(*args, **kwargs):
        raise RuntimeError("empty output after retries")

    monkeypatch.setattr(evaluate, "answer", fail_answer)
    trace_path = tmp_path / "trace.jsonl"
    ...
    assert results[0].failure_stage == "generation"
    assert "empty output after retries" in results[0].failure_reason
    ...
    assert record["failure_classification"]["stage"] == "generation"
