...
| **27. Artifacts LanceDB + UI polish** | `artifacts.py` : `_DIR_ARTIFACTS`, `_OPTIONAL_DIR_ARTIFACTS`, `_dir_artifact_present()`, `_download_dir_artifact()` (HF Hub `list_repo_files` + `hf_hub_download`), `upload_artifacts()` avec `upload_folder()` ; 7 tests `TestDirArtifacts` ; `ui/app.py` : option LanceDB dans le selecteur retriever (defaut si dispo), info bloc mis a jour (49 263 chunks, Solon-1024d), compteur d'echanges en cours, `<details>` texte complet dans les cartes sources | **Done (2026-03-10)** | Faible | 128 → **135 tests** ; index `.lance` survivra aux deploys Streamlit Cloud via HF Hub ; UI: LanceDB selectionne par defaut, source cards extensibles |
| **27c. Benchmark end-to-end LanceDB** | `eval/evaluate.py --retriever lancedb --k 5 --prompt-version 3` — 100Q, copilot/gpt-4.1, 4 workers ; resultats dans `eval/results/copilot-lancedb/eval_20260311_000535.jsonl` | **Done (2026-03-11)** | Faible | **KW=0.676 / F1=0.263 / CitFaith=0.990 / CtxCov=0.733 / Lat=9.28s** — KW +6.8pp vs hybrid k=8 (0.608) ; F1 en retrait −5.5pp (0.318→0.263) ; latence 9.28s vs 15.1s (hybrid k=8) ; CtxCov stable ; trade-off KW↑ vs F1↓ attendu (RRF+FTS favorise le rappel lexical au detriment de la concision) |
| **28. LanceDB+Graph retriever** | `retrievers/lancedb_graph_rag.py` : LanceDB hybrid (vector+FTS+RRF) comme seed → normalisation scores [0.5, 1.0] → expansion Neo4j (CITE, CITE_ARTICLE, MODIFIE, VOIR_ARTICLE) → pool merge → top-k ; cache `_get_chunks_by_doc()` module-level pour 49k rows ; fallback seed-only si Neo4j indisponible ; integration `evaluate.py --retriever lancedb_graph` + `ui/app.py` (nouvelle option LanceDB+Graph par defaut si dispo) ; 16 tests `test_lancedb_graph_rag.py` (TestConstants, TestChunksByDocCache, TestLanceDbGraphRetrieve) | **Done (2026-03-11)** | Moyen | **KW=0.658 / F1=0.263 / CitFaith=0.980 / CtxCov=0.735 / Lat=8.93s** — expansion Neo4j n'ameliore pas F1 vs LanceDB pur (memes 26.3%) ; hypothese : les chunks voisins Neo4j n'alignent pas les mots-cles de reference ; KW −1.8pp vs LanceDB pur (67.6→65.8%) ; 135 → **153 tests** ; infrastructure LanceDB+Graph operationnelle pour variantes futures |
| **29. AutoEval (autoresearch-inspired)** | `autoeval/program.md` : instructions agent autonome (objectif, espace de parametres, regles, stratege hints) ; `autoeval/experiment.py` : fichier unique editable par l'agent — parametres tunables (RETRIEVER, K, VECTOR_WEIGHT, FTS_WEIGHT, USE_RERANKER, QUERY_EXPANSION, etc.) + runner auto qui appelle `eval.evaluate`, imprime metriques et appende `results.tsv` ; `autoeval/.gitignore` : exclut results.tsv ; inspiré de [karpathy/autoresearch](https://github.com/karpathy/autoresearch) — meme pattern 3 fichiers (program.md = instructions, experiment.py = code editable, results.tsv = log) | **Done (2026-03-11)** | Faible | Infrastructure d'optimisation autonome : l'agent lit `program.md`, edite les params dans `experiment.py`, lance `python autoeval/experiment.py` (~30s retrieval-only), lit KW/F1, itere ; cible : KW≥67% ET F1≥30% simultanément ; `--full` pour eval LLM complete (~15min) ; score composite = 0.5×KW + 0.5×F1 ; monkey-patch des constantes LanceDB (VECTOR_WEIGHT/FTS_WEIGHT) pour exploration RRF |
